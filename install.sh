...
readonly CONFIGS=(btop cava fastfetch fish foot hypr kitty rofi swaync wallpapers wallust waybar wlogout themes)
...
generate_hardware_lua() {
    ...
    local tmp_out
    tmp_out="$(mktemp "${out}.nks.XXXXXX")"
    {
        printf '%s\n' '-- Auto-generated by NeKoRoSHELL. Do not edit manually.'
        printf '%s\n' '-- This file intentionally contains only direct Lua assignments/calls.'
        printf '%s\n' '-- It never performs arithmetic on connector names or unset variables.'
        case "$profile" in
    ...
    chown "$TARGET_USER":"$(id -gn "$TARGET_USER")" "$tmp_out"
    chmod 0644 "$tmp_out"
    mv -f -- "$tmp_out" "$out"
...
RC
        ...
        if [[ -x /usr/bin/fish ]]; then
            sudo usermod -s /usr/bin/fish "$TARGET_USER"
        fi
    else
        log_info "Would update $zshrc and set /usr/bin/fish as the login shell."
    fi
}
...
install_home_files() {
    ...
    if (( ! DRY_RUN )); then
        ...
        cp -f "$SCRIPT_DIR/.config/starship.toml" "$TARGET_HOME/.config/starship.toml"
        chown "$TARGET_USER":"$(id -gn "$TARGET_USER")" "$TARGET_HOME/.config/starship.toml"
        chmod 0644 "$TARGET_HOME/.config/starship.toml"
    fi
...
create_backup() {
    ...
    for item in .bashrc .zshrc .p10k.zsh .face.icon change-avatar.sh .config/starship.toml; do
...
validate_source_tree() {
    ...
    for bad_official in bibata-cursor-theme font-awesome wlogout wallust-git mpvpaper mpvpaper-stop-git tty-clock-git awww-git; do
...
verify_nks_tree() {
    ...
    command -v foot >/dev/null 2>&1 || die "foot is missing from the system."
    command -v fish >/dev/null 2>&1 || die "fish is missing from the system."
    command -v starship >/dev/null 2>&1 || die "starship is missing from the system."
