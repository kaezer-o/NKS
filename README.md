...
- Caelestia-style Foot terminal with Fish, Starship, Zoxide, Eza and Direnv
- Powerlevel10k for Zsh as a retained fallback shell configuration
...
The default terminal is Foot, following the current Caelestia terminal setup, with Fish as its shell. NKS uses the Caelestia Fish workflow (Starship prompt, Zoxide, Eza, Direnv, Git abbreviations and foot prompt markers) without installing or depending on the Caelestia desktop shell.
...
| NVIDIA generation | NKS path |
|---|---|
| Turing, Ampere, Ada, Blackwell and newer supported generations | `nvidia-open` on the standard `linux` kernel, otherwise `nvidia-open-dkms` |
| Maxwell, Pascal, Volta | `nvidia-580xx-dkms` from the AUR |
| Unknown generation | `nouveau` fallback |
| Mixed modern + Maxwell/Pascal/Volta NVIDIA generations | `nouveau` fallback |

Concrete examples:

- **GeForce GTX 1660 Ti** is Turing, so the installer selects the current `nvidia-open` path on a standard `linux` installation.
- **GeForce GTX 1060 6 GB** is Pascal, so the installer selects the `nvidia-580xx-dkms` AUR path.
...
- **Caelestia dots** — terminal configuration reference for the Foot + Fish workflow. NKS uses this terminal setup selectively and does not install the Caelestia desktop shell.
...
- SwayNC
- Rofi
- Foot terminal (default terminal)
- Fish shell (default interactive/login shell)
- Kitty (retained as a compatibility/fallback terminal)
- awww
- Wallust
- Hyprshot
- mpvpaper
