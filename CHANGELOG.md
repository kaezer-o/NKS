# 1.6.2 - Hardware recovery and Caelestia terminal integration

- Fixed the generated Hyprland hardware profile so connector names such as `eDP-1` are never treated as numeric Lua values.
- Hardware profile publication is now atomic.
- Hyprland now ignores a malformed machine-specific `hardware.lua` instead of turning the entire configuration into emergency mode; the generic preferred-monitor fallback remains available.
- Corrected the Arch Font Awesome package target from the split-package base name `font-awesome` to the installable `otf-font-awesome`.
- Added Foot as the default NKS terminal and Fish as its shell.
- Added the Caelestia-style Fish workflow with Starship, Zoxide, Eza, Direnv and Git abbreviations.
- Added Wallust-driven Foot colours while retaining the exact inverse foreground/cursor rule used by the other NKS UI components.
- Retained Kitty and the existing Zsh/Powerlevel10k setup as compatibility/fallback components.
